...
multiYChart.prototype = {
    ...
    initialize: function (options, dataSource) {
        let that = this;
        this.id = options.id;
    ...
    createUI: function () {
        ...
        function zoomed() {
            ...
            // 原点x,y位移
            let panX = d3.event.transform.x;
            let panY = d3.event.transform.y;
            // 缩放比例
            let scaleMultiplier = d3.event.transform.k;
            ...
            that.svg
                ...
                .attr('transform', function (d, i) {
                    let move = -i * that.height * scaleMultiplier / 8;
                    if (i === 0) {
                        move -= 10;
                    } else if (i === 8) {
                        move += 10;
    ...
    drowDataAxis: function (info, index) {
        ...
        let type = info.code;
        let axisData = [];
        let diff = (info.max - info.min) / 8;

        for (let i = 0; i < 9; i++) {
            axisData.push(info.min + i * diff);
        }

        let title = info.desc + info.unit;

        let wrap = this.svg.select('g.yAxisWrap');
    ...
    drowDataCircles: function (info) {
        ...
        this.canvas
            ...
            .append('title')
            .text(function (d) {
                let content = '时间：' + that.format(d.datetime, 'yyyy-MM-dd hh:mm') + '\n' + info.desc + '：' + d.value + info.unit;
                return content;
            });
    },

    getConvertValue: function (value, min, max, height) {
        let factor = height / (max - min);
